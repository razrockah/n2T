...
# ╔═╡ 6c30fcae-9a48-11f1-b029-d599e8d29fed
using CSV, DataFrames, CairoMakie

# ╔═╡ 2cb6e5d8-12b1-41b2-94f3-1ea3f066b63b
tbr_df_to_mat(df) = unique(df.x), unique(df.y), reshape(df.mean, length(unique(df.x)), length(unique(df.y)))

# ╔═╡ 3be18d17-1b32-427c-bad0-1e5c620116ea
begin
    li_mesh_tbr_df = CSV.read("li_tbr_mesh.csv", DataFrame)
    li_x, li_y, li_mat = tbr_df_to_mat(li_mesh_tbr_df)
    with_theme(theme_latexfonts()) do
        li_fig = Figure(size = (750,600))
        li_ax =  Axis(li_fig[1, 1]; title = "TBR in Li", xlabel = "x [cm]", ylabel = "y [cm]", titlesize = 30, xlabelsize = 25, ylabelsize = 25)
        hm = heatmap!(li_ax, li_x, li_y, li_mat; colormap = :inferno)
        Colorbar(li_fig[1, 2], hm; label = L"$^3$H per source neutron per cell")
        li_fig
    end
end
...
begin
    flibe_mesh_tbr_df = CSV.read("flibe_tbr_mesh.csv", DataFrame)
    flibe_x, flibe_y, flibe_mat = tbr_df_to_mat(flibe_mesh_tbr_df)
    with_theme(theme_latexfonts()) do
        flibe_fig = Figure(size = (750,600))
        flibe_ax =  Axis(flibe_fig[1, 1]; title = "TBR in FLiBe", xlabel = "x [cm]", ylabel = "y [cm]", titlesize = 30, xlabelsize = 25, ylabelsize = 25)
        hm = heatmap!(flibe_ax, flibe_x, flibe_y, flibe_mat; colormap = :inferno)
        Colorbar(flibe_fig[1, 2], hm; label = L"$^3$H per source neutron per cell")
        flibe_fig
    end
end
...
begin
    pbli_mesh_tbr_df = CSV.read("pbli_tbr_mesh.csv", DataFrame)
    pbli_x, pbli_y, pbli_mat = tbr_df_to_mat(pbli_mesh_tbr_df)
    with_theme(theme_latexfonts()) do
        pbli_fig = Figure(size = (750,600))
        pbli_ax =  Axis(pbli_fig[1, 1]; title = "TBR in PbLi", xlabel = "x [cm]", ylabel = "y [cm]", titlesize = 30, xlabelsize = 25, ylabelsize = 25)
        hm = heatmap!(pbli_ax, pbli_x, pbli_y, pbli_mat; colormap = :inferno)
        Colorbar(pbli_fig[1, 2], hm; label = L"$^3$H per source neutron per cell")
        pbli_fig
    end
end
...
manifest_format = "2.0"
project_hash = "87f704b76d99ea44fea364415e9092f1d8142288"
